...
export const geoCache = new Map<string, THREE.BufferGeometry>();
export const matCache = new Map<string, THREE.Material>();

let cachedAuroraMat: THREE.MeshBasicMaterial | null = null;

export function getGeo(key: string, createFn: () => THREE.BufferGeometry): THREE.BufferGeometry {
    if (!geoCache.has(key)) geoCache.set(key, createFn());
    return geoCache.get(key)!;
}

export function getMat(key: string, createFn: () => THREE.Material): THREE.Material {
...
export class TownEnvironment {
    ...
    public createElvenKingdom(xPos: number, zPos: number) {
        ...
        const baseLeafGeo = getGeo('aethel_leaf_base', () => new THREE.IcosahedronGeometry(1, 1));
        
        for (let i = 0; i < 15; i++) {
            const leafSize = 8 + Math.random() * 6;
            const leaf = new THREE.Mesh(baseLeafGeo, i % 3 === 0 ? glowingLeafMat : leafMat);
        ...
        const topLeaf = new THREE.Mesh(baseLeafGeo, leafMat);
        topLeaf.scale.setScalar(18);
    ...
    public createGrandTavern(xOffset: number, zOffset: number, rotation: number) {
        ...
        const sKnob = new THREE.Mesh(getGeo('sm_knob', () => new THREE.SphereGeometry(0.2)), getMat('sm_m', () => new THREE.MeshStandardMaterial({ color: 0xaa2222 }))); 
