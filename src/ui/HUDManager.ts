    ...
    if (!document.getElementById("chunky-ui-styles")) {
        ...
        style.innerHTML = `
            ...
            @media (max-width: 1200px) {
                ...
                #hud-bottom-center { transform: translateX(-50%) scale(0.85); transform-origin: bottom center; bottom: 85px !important; }
            }

            @media (max-width: 900px) {
                #hud-bottom-right { transform: scale(0.75); transform-origin: bottom right; }
                #hud-bottom-center { bottom: 75px !important; }
                #event-log-container { width: 250px; }
            }
...
export function ensureOverlay(getActiveRoom: () => any, getActionContext: () => any): HTMLDivElement {
    ...
    // 1. ACTION OVERLAY (Moved down to clear the Familiar & Buffs)
    ...
    if (!overlay) {
        ...
        infoPanel.className = "chunky-panel";
        ...
        rosterDiv.className = "chunky-panel";
    ...
    // 2. FAMILIAR FRAME (Tucked neatly under the player bars)
    ...
    if (!familiarFrame) {
        ...
        familiarFrame.className = "chunky-panel";
        familiarFrame.style.position = "fixed";
        familiarFrame.style.top = "155px"; 
        familiarFrame.style.left = "35px"; 
        familiarFrame.style.width = "250px"; 
        familiarFrame.style.boxSizing = "border-box";
        ...
        familiarFrame.innerHTML = `
            ...
            <div style="width: 100%; background: #0f172a; border: 3px solid #334155; border-radius: 8px; padding: 2px; position: relative; height: 14px; box-sizing: border-box;">
                <div id="fam-hp-bar" style="width: 100%; height: 100%; background: #d946ef; border-radius: 4px; transition: width 0.2s;"></div>
                <div id="fam-status-text" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; text-align: center; color: white; font-size: 11px; font-weight: 900; line-height: 10px;"></div>
            </div>
        `;
        document.body.appendChild(familiarFrame);
    }

    // 3. BUFF CONTAINER (Sandwiched between the Familiar and the Context Text)
    ...
    if (!buffContainer) {
        ...
        buffContainer.style.top = "255px"; 
        buffContainer.style.left = "35px";
        buffContainer.style.display = "flex";
        buffContainer.style.flexWrap = "wrap";
        buffContainer.style.gap = "8px";
        buffContainer.style.maxWidth = "250px";
    ...
    if (!globalEventBanner) {
        ...
        globalEventBanner.className = "chunky-panel";
    ...
    // TOP-RIGHT FLEX COLUMN
    let topRightDiv = document.getElementById("hud-top-right");
    if (!topRightDiv) {
        ...
        topRightDiv.style.flexDirection = "column";
        topRightDiv.style.alignItems = "flex-end";
    ...
    let rankCoinsDiv = document.getElementById("hud-rank-coins");
    if (!rankCoinsDiv) {
        rankCoinsDiv = document.createElement("div");
        rankCoinsDiv.id = "hud-rank-coins";
        topRightDiv.appendChild(rankCoinsDiv);
    }
    ...
    if (!minimapContainer) {
        ...
        topRightDiv.appendChild(minimapContainer);
    }
    ...
    if (!coordDiv) {
        ...
        topRightDiv.appendChild(coordDiv);
    }

    let zonePopup = document.getElementById("zone-popup");
    if (!zonePopup) {
    ...
    const worldMapContainer = document.getElementById("world-map-modal") || document.createElement("div");
    ...
    if (!bottomCenterContainer) {
        ...
        auraDiv.id = "hud-aura";
        auraDiv.className = "chunky-panel";
    ...
    if (!medUI) {
        ...
        medUI.innerHTML = `
            <div id="med-main-content">
                ...
                <div id="med-question-container" style="font-size: 24px; font-weight: 900; color: #ffffff; line-height: 1.8; text-align: center; margin-bottom: 20px;"></div>
                
                <input type="text" id="meditation-answer" class="hud-input" placeholder="Type answer..." style="width: 100%; box-sizing: border-box; text-align: center; font-size: 18px; padding: 15px; margin-bottom: 10px;">
                <button id="meditation-submit-btn" class="btn-chunky btn-green" style="width: 100%; padding: 15px;">Focus Intent</button>
            </div>

            <div id="med-upgrade-content" style="display: none; text-align: center;">
                <h2 class="hud-header text-amber text-xl" style="margin-top: 0; letter-spacing: 2px;"><i class="fa-solid fa-bolt"></i> AURA ASCENSION</h2>
                <p class="text-muted text-sm" style="margin-bottom: 25px;">Your focus has reached a milestone. Choose your path:</p>
                
                <button id="btn-upgrade-strength" class="btn-chunky btn-red" style="width: 100%; padding: 15px; margin-bottom: 15px;">
                    Path of Strength (+Aura Output)
                </button>
                <button id="btn-upgrade-control" class="btn-chunky btn-blue" style="width: 100%; padding: 15px;">
                    Path of Control (+Aura Efficiency)
                </button>
            </div>
            
            <div id="med-feedback" class="text-md" style="margin-top: 15px; text-align: center; min-height: 24px;"></div>
        `;
        ...
        const submitAnswer = () => {
            ...
            if (room) {
                const qIndex = parseInt(document.getElementById("med-question-container")!.getAttribute("data-qindex") || "0");
                room.send("submit_meditation", { answer: answer, index: qIndex });
            }
            input.value = "";
            input.disabled = true;
        };

        document.getElementById("meditation-submit-btn")!.onclick = submitAnswer;
        document.getElementById("meditation-answer")!.addEventListener("keydown", (e) => {
            if (e.key === "Enter") {
                e.preventDefault();
                submitAnswer();
            }
        ...
        document.getElementById("btn-upgrade-strength")!.onclick = () => {
            getActiveRoom()?.send("choose_aura_upgrade", { choice: "strength" });
        ...
        document.getElementById("btn-upgrade-control")!.onclick = () => {
            getActiveRoom()?.send("choose_aura_upgrade", { choice: "control" });
            document.getElementById("med-main-content")!.style.display = "block";
            document.getElementById("med-upgrade-content")!.style.display = "none";
            document.getElementById("med-feedback")!.innerText = "";
        };
    ...
    if(!bottomRightContainer) {
        ...
        bottomRightContainer.innerHTML = `
            ...
            <div id="controls-combat" class="controls-group" style="text-align: right;">
                ...
                <div class="controls-row"><span class="text-muted text-sm">Sprint</span> <span class="hud-key">SHIFT</span></div>
                <div class="controls-row"><span class="text-muted text-sm">Dodge</span> <span class="hud-key">SPACE</span></div>
                <div class="controls-row"><span class="text-muted text-sm">Powers Menu</span> <span class="hud-key">K</span></div>
            ...
            <div id="controls-other" class="controls-group" style="display: none; text-align: right;">
                ...
                <div class="controls-row"><span class="text-muted text-sm">Quick Chat</span> <span class="hud-key">TAB</span></div>
...
export function updateHUD(
   ...
   const eventDiv = document.getElementById("global-event-banner");
    ...
    const rankCoinsDiv = document.getElementById("hud-rank-coins");
    if (rankCoinsDiv) {
        ...
        rankCoinsDiv.innerHTML = `
            <div class="chunky-panel" style="padding: 12px 20px; display: flex; gap: 20px; align-items: center; border-width: 4px; border-color: ${rCol};">
    ...
    if (textDiv) {
        ...
        if (me.isSleeping) {
            overlayText = `
            <div class="chunky-panel animate-pulse" style="border-color: #22c55e; text-align: center; margin-top: 15px;">
            ...
            overlayText = `
            <div class="chunky-panel animate-pulse" style="border-color: #38bdf8; text-align: center; margin-top: 15px;">
